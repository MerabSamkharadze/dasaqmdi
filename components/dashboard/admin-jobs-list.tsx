...
export function AdminJobsList({ jobs, locale, translations: t }: AdminJobsListProps) {
  const items = jobs.map((job) => {
    ...
    return {
      id: job.id,
      node: (
        <div className="flex items-center gap-4 rounded-xl border border-border/60 bg-card px-5 py-3.5 shadow-soft">
          <div className="flex-1 min-w-0">
            <Link
              href={`/jobs/${job.id}`}
              className="text-[13px] font-medium truncate hover:text-primary transition-colors block"
            >
              {job.title}
            </Link>
