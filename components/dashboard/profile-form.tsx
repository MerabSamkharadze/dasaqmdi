      ...
      {categories.length > 0 && (
        <div className="pt-4 border-t border-border/60 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {categories.map((cat) => {
              const isSelected = (profile.preferred_categories ?? []).includes(cat.slug);
              return (
                <label
                  key={cat.slug}
                  className={`flex items-center gap-2 rounded-lg border px-3 py-2 cursor-pointer transition-all duration-200 text-[13px] ${
                    isSelected
